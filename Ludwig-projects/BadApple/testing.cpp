...
each pixel in a given matrix. The first parameter is an OpenCV
Matrix of the image with the right size; the second parameter is a
boolean that controls whether to use 70 different characters
in the string for more possible colors instead of 10 for
sharper outputs.
*/
string GetText(Mat, bool);
...
int main()
{
	...
	while (vid.read(frame))
	{
		...
		resize(frame, frame, Size(), ratio2, ratio2/2, INTER_AREA);	//Resize the image
		...
		double framerate = 1.0 / vid.get(CAP_PROP_FPS) - (end - begin).count() / 1e9;
